...
class BotEngine(
        ...
        scope.launch {
            while (isActive) {
                try {
                    delay(60_000)
                    fetchBalance()
                } catch (e: CancellationException) { Log.d(TAG, "Цикл баланса остановлен"); break
                } catch (e: Exception) { Log.e(TAG, "Ошибка в цикле баланса: ${e.message}") }
            }
        }
    }
        ...
        scope.launch {
            ...
            while (isActive) {
                try {
                    fetchBets()
                    delay(30_000)
                } catch (e: CancellationException) { Log.d(TAG, "Цикл ставок остановлен"); break
                } catch (e: Exception) { Log.e(TAG, "Ошибка в цикле ставок: ${e.message}") }
            }
        }
    }
        ...
        scope.launch {
            ...
            while (isActive) {
                try {
                    ...
                    delay(60_000)
                } catch (e: CancellationException) { Log.d(TAG, "Цикл счёта остановлен"); break
                } catch (e: Exception) { Log.e(TAG, "Ошибка в цикле счёта: ${e.message}") }
            }
        }
    }
        ...
        scope.launch {
            ...
            while (isActive) {
                try {
                    ...
                    delay(3_600_000)
                } catch (e: CancellationException) { Log.d(TAG, "Цикл замены остановлен"); break
                } catch (e: Exception) { Log.e(TAG, "Ошибка в цикле замены: ${e.message}") }
            }
        }
    }
    ...
    private suspend fun fetchBalance() {
        ...
        suspendCancellableCoroutine<Unit> { continuation ->
            apiClient.getSaldo(
                cookies = cookies, fsid = data.fsid, deviceId = data.deviceId,
    ...
    private suspend fun fetchBets() {
            ...
            suspendCancellableCoroutine<Unit> { continuation ->
                apiClient.getBets(
                    ...
                    onSuccess = { betDataList ->
                        if (betDataList.isEmpty()) { continuation.resume(Unit); return@getBets }
                        ...
                        if (isExpressAlreadyExists(user.id, matchIds)) {
                            ...
                            continuation.resume(Unit); return@getBets
                        }
                        onLogUpdate?.invoke("[${getCurrentTime()}] 🎲 Получен экспресс: $key")
                        betDataList.forEach { betData ->
                            onLogUpdate?.invoke("[${getCurrentTime()}] 📊 Матч #${betData.mId}: ${betData.home} vs ${betData.away} | Кэф: ${"%.2f".format(betData.startKf)} | Тип: ${typeName(betData.type)}")
                        }
                        ...
                        if (expressDbId > 0) {
                            CoroutineScope(Dispatchers.IO).launch { placeBet(expressDbId, newExpId, user.id, betDataList) }
                        }
                        continuation.resume(Unit)
                    },
                    onError = { error ->
        ...
        } catch (e: Exception) { Log.e(TAG, "fetchBets error", e) }
    }

    private suspend fun placeBet(expressDbId: Long, expId: Int, userId: Long, betDataList: List<ApiClient.BetData>) {
    ...
    private fun placeTestBet(expressDbId: Long, expId: Int, userId: Long, betDataList: List<ApiClient.BetData>) {
        val db = dbHelper.writableDatabase
        val cursor = db.query("express_bets", arrayOf("sumbet", "kfall", "potential_win"), "id = ?", arrayOf(expressDbId.toString()), null, null, null)
        if (!cursor.moveToFirst()) { cursor.close(); return }
        val betAmount = cursor.getDouble(0); val totalKef = cursor.getDouble(1); val potentialWin = cursor.getDouble(2)
        cursor.close()
        val matchesInfo = betDataList.joinToString(" + ") { "${it.home.take(15)} vs ${it.away.take(15)}" }
        onLogUpdate?.invoke("[${getCurrentTime()}] 🧪 ТЕСТ #$expId: ${betAmount.toInt()}₽ × ${"%.2f".format(totalKef)} = ${"%.2f".format(potentialWin)}₽ | $matchesInfo")
        onScoresUpdate?.invoke("🧪 ТЕСТ #$expId: ${betAmount.toInt()}₽ × ${"%.2f".format(totalKef)} = ${"%.2f".format(potentialWin)}₽")
        dbHelper.addLog(userId, "test_express", "ТЕСТ #$expId: ${betAmount.toInt()}₽", context = "test_mode")
        showBetNotification("🧪 ТЕСТ: Экспресс #$expId", "${betAmount.toInt()}₽ | Кэф: ${"%.2f".format(totalKef)} | $matchesInfo")
    }

    private suspend fun placeRealBet(expressDbId: Long, expId: Int, userId: Long, betDataList: List<ApiClient.BetData>) {
        val data = authData ?: return
        val cookies = getCookies()
        if (cookies.isEmpty()) { onLogUpdate?.invoke("[${getCurrentTime()}] ❌ Нет cookies для реальной ставки"); return }

        val clientId = getClientId()
        onLogUpdate?.invoke("[${getCurrentTime()}] 📋 Запрос betSlipInfo (clientId=$clientId)...")

        apiClient.getBetSlipInfo(
            bets = betDataList, cookies = cookies, fsid = data.fsid, deviceId = data.deviceId, clientId = clientId,
            onSuccess = { slipInfo ->
                ...
                if (betAmount < slipInfo.minSum) onLogUpdate?.invoke("[${getCurrentTime()}] ⚠️ Сумма ${betAmount.toInt()}₽ меньше мин. ${slipInfo.minSum.toInt()}₽")

                apiClient.getBetRequestId(
                    cookies = cookies, fsid = data.fsid, deviceId = data.deviceId, clientId = clientId,
                    onSuccess = { requestId ->
                        onLogUpdate?.invoke("[${getCurrentTime()}] ✅ requestId: $requestId")
                        apiClient.placeRealBet(
                            requestId = requestId, betSlipInfo = slipInfo, amount = betAmount,
                            bets = betDataList, cookies = cookies, fsid = data.fsid, clientId = clientId,
                                ...
                                if (placeResponse.result == "betDelay") {
                                    ...
                                    CoroutineScope(Dispatchers.IO).launch {
                                        ...
                                        apiClient.getBetResult(
                                            requestId = requestId, cookies = cookies, fsid = data.fsid,
                                            deviceId = data.deviceId, clientId = clientId,
                                            onSuccess = { result -> processRealBetResult(expressDbId, expId, userId, betDataList, betAmount, slipInfo.totalK, result) },
                                            onError = { error -> onLogUpdate?.invoke("[${getCurrentTime()}] ❌ betResult ошибка: $error"); dbHelper.addLog(userId, "bet_error", "betResult: $error", "ERROR") }
                                        )
                                    }
                                } else {
                            ...
                            onError = { error -> onLogUpdate?.invoke("[${getCurrentTime()}] ❌ placeBet: $error"); dbHelper.addLog(userId, "bet_error", "placeBet: $error", "ERROR") }
                        )
                    },
                    onError = { error -> onLogUpdate?.invoke("[${getCurrentTime()}] ❌ requestId: $error"); dbHelper.addLog(userId, "bet_error", "requestId: $error", "ERROR") }
                )
            },
            onError = { error -> onLogUpdate?.invoke("[${getCurrentTime()}] ❌ betSlipInfo: $error"); dbHelper.addLog(userId, "bet_error", "betSlipInfo: $error", "ERROR") }
        )
    }

    private fun processRealBetResult(expressDbId: Long, expId: Int, userId: Long, betDataList: List<ApiClient.BetData>, betAmount: Double, totalKef: Double, resultResponse: ApiClient.BetResultResponse) {
        ...
        if (result == "couponResult") {
            ...
            if (resultCode == 0) {
                dbHelper.writableDatabase.update("express_bets", ContentValues().apply {
                    put("is_bet_placed", 1); put("sts_all", 1); put("strategy", "real"); put("updated_at", System.currentTimeMillis() / 1000)
                ...
                onLogUpdate?.invoke("[${getCurrentTime()}] $message"); onScoresUpdate?.invoke(message)
                dbHelper.addLog(userId, "bet_placed", "РЕАЛ #$expId: ${betAmount.toInt()}₽, кэф ${"%.2f".format(totalKef)}, №$regId, баланс: ${"%.2f".format(saldo)}₽")
                showBetNotification("✅ Ставка #$expId принята!", "${betAmount.toInt()}₽ | Кэф: ${"%.2f".format(totalKef)} | Выигрыш: ${"%.2f".format(potentialWin)}₽ | №$regId")
                if (saldo > 0) {
                    balance = saldo; BotForegroundService.lastBalance = saldo; onBalanceUpdate?.invoke(saldo)
                ...
                dbHelper.writableDatabase.update("express_bets", ContentValues().apply {
                    put("is_bet_placed", 0); put("sts_all", 0); put("strategy", "real_rejected"); put("updated_at", System.currentTimeMillis() / 1000)
                }, "id = ?", arrayOf(expressDbId.toString()))
    ...
    private suspend fun updateActiveMatchesScores() {
        ...
        val cursor = db.query("express_bets", arrayOf("id", "id_exp", "user_id"), "sts_all IN (0,1,2)", null, null, null, null)
        val expresses = mutableListOf<Triple<Long, Int, Long>>()
        while (cursor.moveToNext()) expresses.add(Triple(cursor.getLong(0), cursor.getInt(1), cursor.getLong(2)))
        cursor.close()
    ...
    private fun getActiveEvents(expressId: Long): List<EventInfo> {
        val db = dbHelper.readableDatabase
        val cursor = db.query("express_events", arrayOf("id", "m_id", "bet_type", "status", "home_score", "away_score", "match_time"),
            "express_id = ? AND is_finalized = 0", arrayOf(expressId.toString()), null, null, null)
        val events = mutableListOf<EventInfo>()
        while (cursor.moveToNext()) events.add(EventInfo(cursor.getLong(0), cursor.getInt(1), cursor.getInt(2), cursor.getInt(3), cursor.getInt(4), cursor.getInt(5), cursor.getInt(6)))
        cursor.close()
        return events
    }
    ...
    private suspend fun updateSingleMatchScore(event: EventInfo, expressDbId: Long, expId: Int, userId: Long) {
        suspendCancellableCoroutine<Unit> { continuation ->
            apiClient.getMatchScore(matchId = event.mId,
                onSuccess = { factors ->
                    if (factors != null) {
                        val sh = factors.score1; val sa = factors.score2; val time = factors.matchTime
                        val updateValues = ContentValues().apply {
                            put("home_score", sh); put("away_score", sa); put("match_time", time)
                            put("updated_at", System.currentTimeMillis() / 1000)
                            if (factors.comand1.isNotEmpty()) put("home_team", factors.comand1)
                            if (factors.comand2.isNotEmpty()) put("away_team", factors.comand2)
                            if (factors.ligaName.isNotEmpty()) put("league_name", factors.ligaName)
                        }
                        dbHelper.writableDatabase.update("express_events", updateValues, "id = ?", arrayOf(event.id.toString()))
                        event.homeScore = sh; event.awayScore = sa; event.matchTime = time
                        val newStatus = checkMatchStatus(event.betType, sh, sa)
                        if (newStatus != event.status) {
                            dbHelper.writableDatabase.update("express_events", ContentValues().apply {
                    ...
                    } else { checkMatchTimeout(event, expressDbId, expId, userId) }
                    continuation.resume(Unit)
                },
                onError = { error -> Log.e(TAG, "Ошибка счёта матча #${event.mId}: $error"); checkMatchTimeout(event, expressDbId, expId, userId); continuation.resume(Unit) }
            )
        }
    }
    ...
    private fun checkMatchTimeout(event: EventInfo, expressDbId: Long, expId: Int, userId: Long) {
        ...
        val db = dbHelper.readableDatabase
        val cursor = db.query("express_events", arrayOf("match_time", "created_at", "home_score", "away_score"), "id = ?", arrayOf(event.id.toString()), null, null, null)
        if (cursor.moveToFirst()) {
            val matchTime = cursor.getInt(0); val createdAt = cursor.getLong(1)
            val homeScore = cursor.getInt(2); val awayScore = cursor.getInt(3)
            cursor.close()
    ...
    private fun checkExpressStatus(expressId: Long, expId: Int, userId: Long) {
        val db = dbHelper.writableDatabase
        val cursor = db.query("express_events", arrayOf("status", "is_finalized"), "express_id = ?", arrayOf(expressId.toString()), null, null, null)
        var anyLost = false; var allWin = true; var hasUnfinished = false; var total = 0
        while (cursor.moveToNext()) {
            total++; if (cursor.getInt(1) == 0) hasUnfinished = true
        ...
        val exprCursor = db.query("express_bets", arrayOf("sumbet", "kfall", "sts_all"), "id = ?", arrayOf(expressId.toString()), null, null, null)
        var betAmount = 0.0; var totalKef = 0.0; var oldStatus = 0
        if (exprCursor.moveToFirst()) { betAmount = exprCursor.getDouble(0); totalKef = exprCursor.getDouble(1); oldStatus = exprCursor.getInt(2) }
        exprCursor.close()
        ...
        if (expressStatus != oldStatus) {
            db.update("express_bets", ContentValues().apply {
                put("sts_all", expressStatus); put("profloss", profitLoss); put("updated_at", System.currentTimeMillis() / 1000)
            ...
            onLogUpdate?.invoke("[${getCurrentTime()}] $message"); onScoresUpdate?.invoke(message)
            dbHelper.addLog(userId, when (expressStatus) { 2 -> "express_win"; 1 -> "express_loss"; else -> "express_active" }, "Экспресс #$expId $statusText, результат: ${"%.2f".format(profitLoss)} ₽")
        }
    }
    ...
    private suspend fun checkAndCreateReplacementExpress() {
        ...
        val cursor = db.rawQuery("SELECT id, id_exp, user_id, sumbet FROM express_bets WHERE sts_all = 1 AND ct < ? AND id_exp_replace = 0 ORDER BY ct ASC LIMIT 1", arrayOf(twelveHoursAgo.toString()))
        if (cursor.moveToFirst()) {
    ...
    private fun getCookies(): Map<String, String> {
        ...
        return if (cookieString.isNotEmpty()) cookieString.split("; ").associate { val parts = it.split("=", limit = 2); parts[0] to (parts.getOrNull(1) ?: "") } else emptyMap()
    }
    ...
    private suspend fun getClientId(): Long {
        ...
        if (cachedId != null) { Log.d(TAG, "getClientId: из БД = $cachedId"); return cachedId }
        ...
        return suspendCancellableCoroutine { continuation ->
            apiClient.getSaldo(
                cookies = getCookies(), fsid = data.fsid, deviceId = data.deviceId,
                onSuccess = { sessionInfo ->
                    val clientId = sessionInfo?.clientId ?: 18845703L
                    user?.let { dbHelper.updateUserInfo(it.id, clientId, sessionInfo?.userName) }
                ...
                onError = { error -> Log.e(TAG, "getClientId ошибка API: $error"); onLogUpdate?.invoke("[${getCurrentTime()}] ❌ Не удалось получить clientId: $error"); continuation.resume(18845703L) }
            )
        }
    }
    ...
    private fun loadBalanceFromDb() {
        authData?.let { data ->
            val user = dbHelper.getUser(data.fsid, data.deviceId)
            user?.let { val stats = dbHelper.getBalanceStats(it.id); if (stats.currentBalance > 0) { balance = stats.currentBalance; BotForegroundService.lastBalance = balance } }
        }
    }

    private fun buildBetSettings(): ApiClient.BetSettings = ApiClient.BetSettings(
        ...
        types = mapOf(
            924 to ApiClient.TypeSettings("1х/футбол/хоккей", prefs.getFloat("type_924_min", 1.15f).toDouble(), prefs.getFloat("type_924_max", 1.35f).toDouble(), prefs.getInt("type_924_start", 80), prefs.getInt("type_924_end", 100)),
            927 to ApiClient.TypeSettings("ф1(+1.5)/футбол/хоккей", prefs.getFloat("type_927_min", 1.15f).toDouble(), prefs.getFloat("type_927_max", 1.35f).toDouble(), prefs.getInt("type_927_start", 1), prefs.getInt("type_927_end", 45)),
            928 to ApiClient.TypeSettings("ф2(+1.5)/футбол/хоккей", prefs.getFloat("type_928_min", 1.15f).toDouble(), prefs.getFloat("type_928_max", 1.35f).toDouble(), prefs.getInt("type_928_start", 1), prefs.getInt("type_928_end", 45))
        )
    )

    private fun isExpressAlreadyExists(userId: Long, matchIds: List<Int>): Boolean {
        val db = dbHelper.readableDatabase
        val cursor = db.query("express_bets", arrayOf("id"), "user_id = ? AND sts_all IN (0,1,2)", arrayOf(userId.toString()), null, null, null)
        val expressIds = mutableListOf<Long>()
        while (cursor.moveToNext()) expressIds.add(cursor.getLong(0))
        cursor.close()
        for (eId in expressIds) {
            val evCursor = db.query("express_events", arrayOf("m_id"), "express_id = ?", arrayOf(eId.toString()), null, null, null)
    ...
    private fun saveExpressToDb(userId: Long, expId: Int, betDataList: List<ApiClient.BetData>): Long {
        ...
        val currentTime = System.currentTimeMillis() / 1000
        val betAmount = prefs.getString("bet_amount", "30")?.toDoubleOrNull() ?: 30.0
        var totalKef = 1.0; betDataList.forEach { totalKef *= it.startKf }
        val potentialWin = betAmount * totalKef
        val testMode = prefs.getBoolean("test_mode", true)
        val strategy = if (testMode) "test" else "real_pending"
        val values = ContentValues().apply {
            put("user_id", userId); put("id_exp", expId); put("kfall", totalKef); put("profloss", 0.0)
            put("balans", balance); put("sumbet", betAmount); put("sts_all", 0); put("is_bet_placed", 0)
            put("ct", currentTime); put("strategy", strategy); put("id_exp_replace", 0)
            put("events_count", betDataList.size); put("total_odds", totalKef); put("bet_amount", betAmount)
            put("potential_win", potentialWin); put("balance", balance); put("created_time", currentTime)
            put("created_at", currentTime); put("updated_at", currentTime)
        }
        ...
        betDataList.forEach { bet ->
            ...
            val evValues = ContentValues().apply {
                ...
                put("m_id", bet.mId); put("start_odds", bet.startKf); put("current_odds", bet.lastKf)
                put("bet_type", bet.type); put("status", initialStatus)
                put("home_score", bet.sh); put("away_score", bet.sa); put("match_time", 0)
                put("created_at", currentTime); put("updated_at", currentTime); put("is_finalized", 0)
        ...
        dbHelper.addLog(userId, "express_created", "Экспресс #$expId: ${betDataList.size} матчей, кэф ${"%.2f".format(totalKef)}, ставка ${betAmount.toInt()} ₽")
        return expressId
    }
    ...
    private fun showBetNotification(title: String, content: String) {
        ...
        val notification = NotificationCompat.Builder(context, "bet_channel")
            .setContentTitle(title).setContentText(content).setSmallIcon(android.R.drawable.stat_notify_more)
            .setContentIntent(pendingIntent).setAutoCancel(true).setPriority(NotificationCompat.PRIORITY_HIGH).build()
        notificationManager.notify(System.currentTimeMillis().toInt(), notification)
    }

    private fun getCurrentTime(): String = java.text.SimpleDateFormat("HH:mm:ss", java.util.Locale.getDefault()).format(java.util.Date())

    data class EventInfo(val id: Long, val mId: Int, val betType: Int, var status: Int, var homeScore: Int, var awayScore: Int, var matchTime: Int = 0)
}